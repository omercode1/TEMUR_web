...
import { ProjectInquiry, PreferredContact } from '../../../types/project';
import { PREFERRED_CONTACTS } from '../../../config/projectFormConfig';
...
export function StepContact({ data, update, onNext, onBack }: StepContactProps) {
  ...
  const validateAndNext = () => {
    ...
    if (!data.contact.name.trim() || data.contact.name.length < 2) {
      newErrors.name = 'Lütfen geçerli bir ad soyad girin.';
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!data.contact.email.trim() || !emailRegex.test(data.contact.email)) {
          ...
          <div className="flex flex-wrap gap-4">
            {PREFERRED_CONTACTS.map((pref) => (
              <button
                ...
                className={`px-5 py-3 text-sm tech-corners border transition-all 
