...
export function Services() {
  return (
    <section id="services" className="py-24 md:py-32 relative bg-surface border-t border-white/5">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <p className="text-text-secondary max-w-md text-balance text-lg font-light">
...
function ServiceRow({ service, index }: { service: (typeof services)[number]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="hover-target group relative border-b border-white/5 py-12 md:py-16 flex flex-col lg:flex-row lg:items-center gap-8 cursor-pointer overflow-hidden"
    >
      ...
      <div className="relative z-10 lg:w-1/4">
        <span className="font-mono text-xl text-white/30 group-hover:text-white/60 transition-colors duration-300">
          {service.num}
        </span>
      </div>
      
      <div className="relative z-10 lg:w-2/4">
        <h4 className="text-3xl md:text-5xl font-display font-semibold text-white mb-4">
          {service.title}
        </h4>
        <p className="text-text-secondary leading-relaxed max-w-xl group-hover:text-white transition-colors duration-300 font-light">
          {service.desc}
        </p>
      </div>

      <div className="relative z-10 lg:w-1/4 flex justify-start lg:justify-end">
        {/* Plain div button — removed Magnetic component */}
        <div className="w-16 h-16 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:border-white transition-all duration-300">
          <ArrowRight className="text-white/50 group-hover:text-black group-hover:-rotate-45 transition-all duration-300 w-6 h-6" />
        </div>
      </div>
    </motion.div>
  );
